...
 
int phanLoai(string s){
	if(s[0]=='-') return -1;					//so am
	else if(s[0]>='a' && s[0] <='z') return 0;	// chu cai
	else return 1;								//so duong	
}
 
bool cmp1(string a , string b){					//so duong
	int x=a.length(), y=b.length();
	if (x<y) return true;
	else if (x==y && a<b) return true;
	return false;
}
 
bool cmp2(string a, string b){					//so am 
	int x=a.length(), y=b.length();
	if(x==y && a>b) return false;
	else if(x<y && a>b) return false;
	else if(x>y && a<b) return true;
	return true;
}
 
int main(){
	...
	while(1){
		string s;
		vector<string> ds;
		vector<string> chu,soAm, soDuong;
		while (1){		
			cin>>s;
			ds.push_back(s);
			if (phanLoai(s)==-1) soAm.push_back(s);
			else if (phanLoai(s)==0) chu.push_back(s);
			else soDuong.push_back(s);
			if (s==".") return 0;
			if (s[s.length()-1]=='.') break;
		}
		
		sort(chu.begin(),chu.end());
		sort(soAm.begin(),soAm.end(),cmp2);
		sort(soDuong.begin(),soDuong.end(),cmp1);
		
		vector<string> kq;
		int moveC=0,moveA=0,moveD=0;
		for(int i=0;i<ds.size();i++){
			if (phanLoai(ds[i])==0) kq.push_back(chu[moveC++]);
			else{
				if (moveA<soAm.size())
				kq.push_back(soAm[moveA++]);
				else kq.push_back(soDuong[moveD++]);
			}
		}
		for (int i=0;i<kq.size()-1;i++) kq[i][kq[i].length()-1]=',';			// doi dau
		kq[kq.size()-1][kq[kq.size()-1].length()-1]='.';
		
		
		for(int i=0;i<kq.size();i++){											//xuat
			cout<<kq[i]<<" ";
		}
		cout<<endl;
...
//		cout<<endl;
//		for (int i=0;i<soAm.size();i++) cout<<soAm[i]<<" ";
//		cout<<endl;
//		for (int i=0;i<soDuong.size();i++) cout<<soDuong[i]<<" ";
//		cout<<endl;
	}
return 0;
}
