...
 
	while(1){
		if ( r.empty() && f.empty() ) break;
		if (r[r.size()-1] > f[f.size()-1]){ 	// Luot choi cua Raphtalia 
			goldR+=r[r.size()-1];
			r.pop_back();
		}else{
			f.pop_back();
...
//		xuat(r);
//		xuat(f);
		if (f[f.size()-1] > r[r.size()-1]){ 	// Luot choi cua Raphtalia 
			goldF+=f[f.size()-1];
			f.pop_back();
		}else{
			r.pop_back();
...
//	for (int i=0;i<r.size();i++) goldR+=r[i];
//	for (int i=0;i<f.size();i++) goldF+=f[i];
//	cout<<goldR<<" "<<goldF<<endl;
	cout<<goldR - goldF;
return 0;
}
