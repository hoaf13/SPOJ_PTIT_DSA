...
void update(){
	...
	for(int i=0;i<s.length();i++){		// kiem tra co chu L hay khong
		if (c[i] == 2) flag = true;
	}
	
	ll sum = 1;		// da co 1 chu L
	if (flag){
		for(int i=0;i<s.length()-2;i++){
			if (c[i] == c[i+1] && c[i+1]==c[i+2]) return;
			if (c[i] > 0 && c[i+1] > 0 && c[i+2] >0) return;
		}		
		for(int i=0;i<s.length();i++){
			if (b[i]==-1){
				if (c[i] == 1) sum*=20;
...
//		cout<<bit<<endl;
		...
		bit+="0";	Try(i+1);	bit.erase(bit.end()-1,bit.end());
		bit+="1";	Try(i+1);	bit.erase(bit.end()-1,bit.end());
		bit+="2";	Try(i+1);	bit.erase(bit.end()-1,bit.end());
	}
}
 
// 5 nguyen am - 0
...
int main(){
	...
	for(int i=0;i<s.length();i++){
		...
		if( s[i] == 'A' || s[i] ==  'E' || s[i] == 'I' || s[i] == 'O' || s[i] == 'U' ){	// nguyen am
			b[i]=0;
			continue;
		}
		if( s[i] == 'L'){			// chu L
