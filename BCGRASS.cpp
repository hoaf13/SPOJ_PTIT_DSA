...
 
int row ,col;
char grass[limit][limit];
int dinh[limit][limit];
int check[10004]={0};
 
 
//===================== khoi tao ==================================
...
int xqY[] = {  0, 0, 1,-1};
 
void DFS_LT(int r, int c){ 	// danh dau thanh phan lien thong
	for(int i=0;i<4;i++){
		int X = xqX[i] + r;
		int Y = xqY[i] + c;
		if(check[dinh[X][Y]] == 0 && grass[X][Y] == '#' && X>0 && X <= row && Y > 0 && Y<=col ){
			check[dinh[X][Y]] = 1;
			DFS_LT(X,Y);
		}
	}
