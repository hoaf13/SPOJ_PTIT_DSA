...
// kiem tra xem da TM dk chua
bool test(){
	char l=b[0][0];
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			if(l!=b[i][j]) return false;
		}
	}
	return true;
}
...
void xuly(){
	...
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			if(tg[i][j]==1){
				...
				if(b[i][j+1]=='w') b[i][j+1]='b';
				else  b[i][j+1]='w';
 
				if(b[i][j-1]=='w') b[i][j-1]='b';
				else b[i][j-1]='w';
 
				if(b[i-1][j]=='w') b[i-1][j]='b';
				else b[i-1][j]='w';
	...
	// tim so phep bien doi
 
	if(test()) Min=min(Min,dem);
