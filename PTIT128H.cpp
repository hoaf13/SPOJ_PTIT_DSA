...
bool check(){
	for(int i=0;i<9;i++){
	int dem1[10]={0};	// check hang			
		for (int j=0;j<9;j++){
			dem1[ sudoku[i][j] - '0' ]++;
			if (dem1[ sudoku[i][j] - '0' ] > 1) {
				return false;
			}		
		}
	}
	for(int j=0;j<9;j++){
	int  dem2[10] = {0} ;	// check cot			
		for (int i=0;i<9;i++){
			dem2[ sudoku[i][j] - '0' ]++;
			if (dem2[ sudoku[i][j] - '0' ] > 1) {
				return false;
			}		
		}
	}
	...
	for (int i=0;i<3;i++){	// check khung
		for (int j=0;j<3;j++){
			int  dem3[10] = {0};
			for(int p = xqX[i] ; p<xqX[i] + 3 ; p++ ){
				for (int q = xqY[j]; q <xqY[j] +3 ;q++){
					dem3[sudoku[p][q] - '0' ]++;
					if (dem3[sudoku[p][q] - '0' ] > 1) {
						return false;
					}	
				}
			}		
